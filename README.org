...
translation editor. ~nemo~ is my file manager.

~nodejs_20~ for NPM support for Clojure web applications with Kit.
#+name: applications
#+begin_src text
  reaper
  nyxt
...
(~nixos-help~).
#+begin_src nix :tangle configuration.nix :tangle ./t480/configuration.nix :tangle ./basic/configuration.nix :noweb yes
