...
GNOME (Wayland) works fine for my current use-cases and I am enjoying the
one-workspace per application workflow.
...
also the Nix package manager, NixOS is a Linux distribution based on Nix,
...
*** Proxmox
I could run Proxmox as the base and NixOS on top as a VM with a GPU passed to
it.  This would enable me to run Windows in a VM too with GPU passthrough, I do
not know the limitations of this approach.
...
I'm running the latest stable which is ~23.11~ with [[https://nixos.wiki/wiki/Home_Manager][home-manager]] in my flake. I
...
Enable Electron for [[https://nixos.wiki/wiki/Wayland][Wayland]] by setting ~NIXOS_OZONE_WL~ to "1".
...
- [ ] When upgrading to ~23.11~ make sure to rename ~noto-fonts-emoji~ to
  ~noto-fonts-color-emoji~.  See [[https://nixos.org/manual/nixos/unstable/release-notes#sec-release-23.11-incompatibilities][NixOS 23.11 - Backward Incompatibilities]] and
  [[https://github.com/NixOS/nixpkgs/issues/221181][#221181]].
#+name: fonts
#+begin_src nix
  fonts = {
...
#+begin_src nix
  services.emacs = {
      enable = true;
      package = import /home/adham/.emacs.d {pkgs = pkgs.<<emacs-variation>>};
    };
#+end_src
...
*** Overlay
The overlay is only for Emacs unstable and is currently not in use, to use it add
...
# Local Variables:
# jinx-local-words: "Flatpack Flatpak Hyprland Nixpkgs Optiplex Proxmox Screencast Waydroid Wayland arandr blueman cachix dmenu dunst dwm ffcast flameshot fuzzel hyprland noweb passthrough picom slstatus systemd waybar wdisplays wf"
# End:
